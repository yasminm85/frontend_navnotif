...
export default function KelolaDisplay() {
  ...
  const fetchAgendaSelesai = async () => {
    try {
      const res = await api.get('/api/task/disposisi', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleAddMedia = async (e) => {
    ...
    try {
      ...
      let response = await api.post('/api/media/create-media', formData, {
        headers: {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          Swal.fire('Deleted!', 'Data Media berhasil dihapus.', 'success');
          setMedia((prev) => prev.filter((item) => item._id != id));
        } catch (error) {
          Swal.fire('Error!', 'Gagal Mengahapus Media.', 'error');
        }
      }
    });
  };
          ...
          <TabPanel header="Kelola Media" leftIcon="pi pi-image mr-2">
            <Grid container spacing={2} sx={{ mb: 3 }}>
              ...
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 2, textAlign: 'center', bgcolor: '#f3e5f5' }}>
                  <Typography variant="h6" color="text.secondary">
                    <i className="pi pi-replay mr-2"></i>1 Siklus
                  </Typography>
            ...
            <Card>
              ...
              <DataTable
                ...
                showGridlines
              >
                <Column header="No" body={(data, options) => options.rowIndex + 1} style={{ width: '60px' }} />
                <Column field="filename" header="Nama File" />
                <Column field="mimetype" header="Tipe" body={typeBodyTemplate} style={{ width: '150px' }} />
                <Column field="duration" header="Durasi" body={durationBodyTemplate} style={{ width: '150px' }} />
                <Column header="Action" body={actionBodyTemplate} style={{ width: '220px' }} />
              </DataTable>
            </Card>
          </TabPanel>
            ...
            <Card className="mb-3" style={{ background: '#f0fdf4', border: '1px solid #86efac' }}>
              <div className="flex align-items-center justify-content-between">
                <div>
                  ...
                  {previewCount === 0 && <small style={{ color: 'red' }}>⚠ Tidak ada agenda selesai dalam rentang tanggal ini</small>}
                </div>
      ...
      <Dialog
        ...
        footer={
          <div>
            ...
            />
            <Button label="Tambah" icon="pi pi-check" onClick={handleAddMedia} />
          </div>
        }
      >
