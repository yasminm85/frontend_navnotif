...
export default function DaftarNotifikasi() {
  ...
  const laporanTambahanActionTemplate = (row) => {
    ...
    const laporanUtamaSudahDiisi = row.laporan && row.laporan.trim() !== '';
    return (
      <Button
  ...
  const handleSaveLaporan = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await api.patch(`/api/task/disposisi/${currentTask._id}/laporan`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      console.error('Error update laporan:', err.response?.data || err.message);
      Swal.fire({
        icon: 'error',
  ...
  const handleSaveLaporanTambahan = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await api.patch(`/api/task/disposisi/${currentTaskTambahan._id}/laporan-tambahan`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      Swal.fire({
        icon: 'success',
        title: 'Berhasil',
        text: 'Laporan Tambahan berhasil disimpan'
      });
    } catch (err) {
      console.error('Error update laporan:', err.response?.data || err.message);
      Swal.fire({
  ...
  const fileBodyTemplate = (fileId) => {
    ...
    const handleOpen = async () => {
      try {
        const res = await api.get(`/api/task/file/${fileId}`, {
          responseType: 'blob'
        });
    ...
    return <Button label="Lihat" icon="pi pi-file" className="p-button-text p-button-sm" onClick={handleOpen} />;
  };

  const handleOpenLaporan = async () => {
    try {
      const res = await api.get(`/api/task/file/${currentTask.laporanFileId}`, {
        responseType: 'blob'
      });
  ...
  const handleOpenLaporanTambahan = async () => {
    try {
      const res = await api.get(`/api/task/file/${currentTaskTambahan.laporanFileTambahanId}`, {
        responseType: 'blob'
      });
  ...
  return (
    <div className="card">
      <MainCard title="Daftar Notifikasi">
        <Dialog header="Laporan Tugas" visible={showDialog} style={{ width: '30rem' }} modal onHide={() => setShowDialog(false)}>
          {currentTask && (
            <div className="flex flex-column gap-3">
              ...
              {currentTask.laporanFileId ? (
                <Button label="Lihat File Laporan" icon="pi pi-file" className="p-button p-button-sm mb-2" onClick={handleOpenLaporan} />
              ) : (
              ...
              <div className="flex justify-end gap-2 mt-3">
                {currentTask.laporan_status === 'SUDAH' ? (
                  <Button label="Kembali" className="p-button-text" onClick={() => setShowDialog(false)} />
                ) : (
            ...
            <div className="flex flex-column gap-3">
              ...
              <div className="flex justify-end gap-2 mt-3">
                {currentTaskTambahan?.laporan_tambahan ? (
                  <Button label="Kembali" className="p-button-text" onClick={() => setShowDialogTambahan(false)} />
                ) : (
        ...
        <Dialog header="Detail Notifikasi" visible={showDetail} modal style={{ width: '25rem' }} onHide={() => setShowDetail(false)}>
          {selectedNotif && (
            <div className="flex flex-column gap-2">
              <p>
                <strong>Nama Kegiatan:</strong> {selectedNotif.nama_kegiatan}
              </p>
              <p>
                <strong>Tanggal:</strong> {formDate(selectedNotif.tanggal)}
              </p>
              <p>
                <strong>Jam Mulai:</strong> {formTime(selectedNotif.jam_mulai)}
              </p>
              <p>
                <strong>Jam Selesai:</strong> {formTime(selectedNotif.jam_selesai)}
              </p>
              <p>
                <strong>Tempat:</strong> {selectedNotif.tempat}
              </p>
              <p>
                <strong>File:</strong>
                {fileBodyTemplate(selectedNotif.fileId)}
              </p>
              <p>
                <strong>Catatan:</strong> {selectedNotif.catatan || '-'}
              </p>
            </div>
          )}
        </Dialog>

        <DataTable value={tasks} paginator rows={5} loading={loading} dataKey="_id" emptyMessage="Tidak ada data.">
